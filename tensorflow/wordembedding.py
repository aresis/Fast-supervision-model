...
noise_pattern = re.compile("|".join(["http\S+", "\@\w+", "\#\w+", "\d+", r"'"]))
...
if mode == "chinese":
    ...
    df[4] = df[1].apply(lambda x: ' '.join([word for word in word_tokenize(x) if word.lower() not in stopwords]))
...
def attention_lstm():
    ...
    def create_LSTM_with_attention():
        ...
        embedding = Embedding(MAX_NB_WORDS,
                              EMBEDDING_DIM,
                              weights=[embedding_matrix],
                              input_length=X.shape[1])(x)
        drop = SpatialDropout1D(0.1)(embedding)
        LSTM_layer = LSTM(128, return_sequences=True,recurrent_dropout=0.2)(drop)
